...
flcdata<-subset(countrydata, Country_size > pop_crit_5m & (Pop_flood_risk_2010 / Country_size) > 0.02)
...
flcdata$perc_dflexp_2100<-(flcdata$Pop_flood_risk_2100 - flcdata$Pop_flood_risk_2010) / flcdata$Pop_flood_risk_2010
plot(flcdata$perc_dpop_2100, flcdata$perc_dflexp_2100)
...
flcdata$crit_intens_label<-flcdata$Country_size > 100000000 | (flcdata$perc_dpop_2100>0 & flcdata$perc_dflexp_2100 < 0) | (flcdata$perc_dpop_2100<0 & flcdata$perc_dflexp_2100 > 0)
flcdata$crit_all_label<-flcdata$perc_dflexp_2100 > 2 | flcdata$perc_dpop_2100 > 2.5 | flcdata$Country_size > 250000000 
flcdata$all_label<-""
...
fl_all<-ggplot(flcdata, 
                aes(y=perc_dflexp_2100, x=perc_dpop_2100, color=WB_region, label=all_label)
                #aes(x=growth.in.ls.safe, y=growth.in.ls.unsafe, color=coltest)
)
fl_all=fl_all+
  coord_cartesian(xlim=c(-1,4.78), ylim=c(-1, 4))+
...
#ggsave("Z:/20_Werk/2023_VU_WorldBank/pics/241023/flood_exposure_countries_all.png", width=8, height=8)


fl_zoom<-ggplot(flcdata, 
                    aes(y=perc_dflexp_2100, x=perc_dpop_2100, color=WB_region, label=intens_label)
                    #aes(x=growth.in.ls.safe, y=growth.in.ls.unsafe, color=coltest)
)
fl_zoom=fl_zoom+
...
#ggsave("Z:/20_Werk/2023_VU_WorldBank/pics/241023/flood_exposure_countries_zoomed.png", width=8, height=8)

fl_all + 
  annotation_custom(ggplotGrob(fl_zoom), xmin = 2.2, xmax = 5, ymin = -1.2, ymax = 1.4) +
  geom_rect(aes(xmin = -0.85, xmax = 1.1, ymin = -0.85, ymax = 1.1), color='black', linetype='dotted', alpha=0) +
  geom_rect(aes(xmin = 2.2, xmax = 5, ymin = -1.2, ymax = 1.4), color='black', linetype='dotted', alpha=0) +
  geom_segment(aes(x = 1.1, y = -0.85, xend = 2.2, yend = -1.2), color="black",linetype='dotted') +
  geom_segment(aes(x = 1.1, y = 1.1, xend = 2.2, yend = 1.4), color="black",linetype='dotted')

ggsave("Z:/20_Werk/2023_VU_WorldBank/pics/241023/floods_exposure_countries_exp.png", width=15.8, height=9.524)
